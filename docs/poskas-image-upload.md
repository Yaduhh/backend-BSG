...
const uploadImagesToServer = async (images) => {
  ...
  const response = await fetch('http://192.168.66.232:3000/api/upload/poskas', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
    },
...
### Format URL
```
http://192.168.66.232:3000/uploads/poskas/poskas-{timestamp}-{random}.{ext}
```

### Contoh URL
```
http://192.168.66.232:3000/uploads/poskas/poskas-1703123456789-123456789.jpg
```
...
const renderImage = (image) => {
  const imageUrl = image.url 
    ? `http://192.168.66.232:3000${image.url}`
    : image.uri;
