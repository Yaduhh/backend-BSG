...
// Get all chat rooms for admin (with all users)
router.get('/rooms/:adminId', authenticateToken, authenticateAdmin, async (req, res) => {
  try {
    ...
    const formattedRooms = chatRooms.map(room => {
      const otherUser = room.user1_id == adminId ? room.user2 : room.user1;
      return {
        room_id: `${room.user1_id}_${room.user2_id}`,
        other_user: otherUser,
        last_message: room.last_message,
        last_message_time: room.last_message_time,
        unread_count: room.user1_id == adminId ? room.unread_count_user1 : room.unread_count_user2
      };
    });
...
// Get all users for admin to start new chat
router.get('/contacts/:adminId', authenticateToken, authenticateAdmin, async (req, res) => {
  try {
    const { adminId } = req.params;
...
router.get('/messages/:room_id', authenticateToken, authenticateAdmin, async (req, res) => {
  try {
...
// Send message (admin to any user)
router.post('/send', authenticateToken, authenticateAdmin, async (req, res) => {
  try {
    const { room_id, message, receiver_id } = req.body;
    const sender_id = req.user.id; // Changed from req.user.userId
...
router.put('/read/:room_id', authenticateToken, authenticateAdmin, async (req, res) => {
  try {
    const { room_id } = req.params;
    const adminId = req.user.id;

    // Reset unread count for admin in chat room
    await ChatRoom.update(
      {
