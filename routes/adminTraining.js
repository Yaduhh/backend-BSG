...
// Get all users with training data (admin only - read only)
router.get('/users', authenticateToken, async (req, res) => {
  try {
    ...
    // Calculate training statistics based on filtered users
    ...
    console.error('Error fetching users training data for admin:', error);
    res.status(500).json({
      success: false,
...
// Update user training status (admin only)
router.put('/users/:id/training', authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
    ...
    // Validate that at least one training field is provided
    if (
      training_dasar === undefined && 
      training_leadership === undefined && 
      training_skill === undefined && 
      training_lanjutan === undefined
    ) {
      return res.status(400).json({
        success: false,
        message: 'At least one training field must be provided'
      });
    }
    ...
    const user = await User.findOne({
      where: {
        ...
        status_deleted: false,
        role: {
          [require('sequelize').Op.ne]: 'owner' // Cannot edit owner
        }
      }
    });
    ...
    // Prepare update data
    const updateData = {};
    if (training_dasar !== undefined) updateData.training_dasar = Boolean(training_dasar);
    if (training_leadership !== undefined) updateData.training_leadership = Boolean(training_leadership);
    if (training_skill !== undefined) updateData.training_skill = Boolean(training_skill);
    if (training_lanjutan !== undefined) updateData.training_lanjutan = Boolean(training_lanjutan);
...
router.get('/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Error fetching user training detail for admin:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
